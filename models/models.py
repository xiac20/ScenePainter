...
class FrameSyn(torch.nn.Module):
    def __init__(self, config, inpainter_pipeline, depth_model, vae, rotation,
                 image, adaptive_negative_prompt, BLD):
        super().__init__()
    ...
    def finetune_customization_step(self, inpainted_image, new_recon_image, mask):
        model_pred = new_recon_image
        target = inpainted_image
        model_pred_partial = model_pred * mask
        target_partial = target * mask 
        loss = F.mse_loss(
        ...
        loss = 0.5 * loss + 1 * partial_loss
        
        return loss

    # @torch.no_grad()
    def inpaint(self, rendered_image, inpaint_mask, fill_mask=None, fill_mode = 'cv2_telea', prompt = None, num_of_assets = 5, style_prompt = None, negative_prompt = None):
        # set resolution
        ...
        prompt_pre = 'a photo of '
        holder = '<asset>'
        holder_list = [
            holder.replace(">", f"{idx}>")
            for idx in range(num_of_assets)
        ]
        prompt_end = " and ".join(holder_list)
        prompt = ''
        prompt = prompt_pre + prompt + prompt_end
        if style_prompt is not None:
            prompt = prompt + 'Style: ' + style_prompt
        if negative_prompt is not None:
            prompt = prompt +'Negative Inpainting: ' + negative_prompt
        

        inpainted_image, inpainted_image_latents = self.bld.edit_image(
            init_image,
            mask_image,
            prompts=prompt,
    ...
    def decode_latents_customization(self, latents):
        latents_convert = 1 / 0.18215 * latents.half()

        image = self.bld.vae.decode(latents_convert).sample
...
class KeyframeGen(FrameSyn):
    def __init__(self, config, inpainter_pipeline, mask_generator, depth_model, vae, rotation, 
                 image, base_dir, sidx, fidx, adaptive_negative_prompt="", 
                 segment_model=None, segment_processor=None, BLD=None):
        
        
        self.run_dir = base_dir / f"Gen_s{sidx:02d}_f{fidx:01d}"
...
class KeyframeInterp(FrameSyn):
    def __init__(self, config, inpainter_pipeline, depth_model, vae, rotation, 
                 image, base_dir, sidx, fidx, adaptive_negative_prompt, kf2_upsample_coef=1,
        ...
        self.run_dir = base_dir / f"Interp_s{sidx:02d}_f{fidx:01d}"
        (self.run_dir / 'images').mkdir(parents=True, exist_ok=True)
        ...
        super().__init__(config, inpainter_pipeline, depth_model, vae, rotation,  
                         image, adaptive_negative_prompt)
