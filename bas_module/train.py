...
def auto_detect_num_of_assets(instance_data_dir):
    """
    Automatically detect the number of assets (mask files).
    Returns the total number including the scene (background).
    """
    mask_pattern = os.path.join(instance_data_dir, "mask*.png")
    ...
    if not mask_numbers:
        raise ValueError(f"No mask files (mask*.png) found in directory {instance_data_dir}")
    
    # Return the count of consecutive mask files starting from 0, plus 1 for the scene
    ...
    for i in expected_masks:
        mask_path = os.path.join(instance_data_dir, f"mask{i}.png")
        if not os.path.exists(mask_path):
            raise ValueError(f"Missing mask file: {mask_path}")
    
    # Return total assets count (masks + scene)
    return len(expected_masks) + 1


def auto_detect_fragments(instance_data_dir, num_of_assets):
    """
    Automatically detect fragment masks for each asset.
    Returns a list where:
    - First element is always 0 (representing the scene)
    - Other elements represent the number of fragment masks for each asset
    """
    fragments_list = [0]  # First element is always 0 for scene
    
    # For each asset (starting from mask0), detect fragment masks
    for i in range(num_of_assets - 1):  # -1 because first is scene
        fragment_pattern = os.path.join(instance_data_dir, f"mask{i}_*.png")
        fragment_files = glob.glob(fragment_pattern)
        
        # Extract fragment numbers and find the maximum
        fragment_numbers = []
        for fragment_file in fragment_files:
            filename = os.path.basename(fragment_file)
            # Use regex to extract the fragment number after underscore
            match = re.search(rf"mask{i}_(\d+)\.png", filename)
            if match:
                fragment_num = int(match.group(1))
                fragment_numbers.append(fragment_num)
        
        if fragment_numbers:
            # Number of fragments is max_fragment_number + 1 (since they start from 0)
            num_fragments = max(fragment_numbers) + 1
            # Verify all fragment files from 0 to max exist
            for j in range(num_fragments):
                expected_fragment = os.path.join(instance_data_dir, f"mask{i}_{j}.png")
                if not os.path.exists(expected_fragment):
                    logger.warning(f"Missing fragment file: {expected_fragment}")
                    num_fragments = 0
                    break
        else:
            num_fragments = 0
        
        fragments_list.append(num_fragments)
        logger.info(f"Detected {num_fragments} fragment masks for mask{i}")
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="The weight of masked recon loss.",
    )
    parser.add_argument(
        "--whole_loss_weight",
        type=float,
    ...
    #     type=str,
    #     default="0000",
    #     help="Number of fragments for each asset.",
    # )
    parser.add_argument(
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        self.placeholder_relation_tokens = placeholder_relation_tokens
        ...
        self.num_of_fragments_list = num_of_fragments
        self.instance_fragment_masks = []
        for i in range(num_of_assets-1):
            fragment_mask_list = []
            for j in range(self.num_of_fragments_list[i+1]):
                instance_fragment_mask_path = os.path.join(instance_data_root, f"mask{i}_{j}.png")
                curr_fragment_mask = Image.open(instance_fragment_mask_path)
                curr_fragment_mask = self.mask_transforms(curr_fragment_mask)[0, None, None,...]
                fragment_mask_list.append(curr_fragment_mask)
            self.instance_fragment_masks.append(fragment_mask_list) 
    ...
    def __getitem__(self, index):
        ...
        example["type"] = choiceNum[0]
        # 0 for V v1 ; 1 for v1 v2 ; 2 for v1 v1_sub
        
        if example['type'] == 0:
            tokens_ids_to_use = [0]
            tokens_ids_to_use.append(random.sample(range(1, self.num_of_assets),k=1)[0])
            # tokens_to_use = [self.placeholder_tokens[0], self.placeholder_relation_tokens[self.relation_dict.get(frozenset([0,tokens_ids_to_use[1]]),None)], self.placeholder_tokens[tokens_ids_to_use[1]]]
            tokens_to_use = [self.placeholder_tokens[0], self.placeholder_tokens[tokens_ids_to_use[1]]]
            example["recon_masks"] = torch.stack([torch.ones_like(self.instance_masks[0]), self.instance_masks[tokens_ids_to_use[1]-1]])
            example["prompt_masks"] = torch.stack([torch.ones_like(self.instance_masks[0]), self.instance_masks[tokens_ids_to_use[1]-1]])
        elif example['type'] == 1:
            tokens_ids_to_use = random.sample(range(1, len(self.placeholder_tokens)), k=2)  
            # tokens_to_use = [self.placeholder_tokens[tokens_ids_to_use[0]], self.placeholder_relation_tokens[self.relation_dict.get(frozenset([tokens_ids_to_use[0], tokens_ids_to_use[1]]),None)], self.placeholder_tokens[tokens_ids_to_use[1]]]
            tokens_to_use = [self.placeholder_tokens[tokens_ids_to_use[0]], self.placeholder_tokens[tokens_ids_to_use[1]]]
            tokens_ids_to_use_cvt = [token_id-1 for token_id in tokens_ids_to_use]
            example["prompt_masks"] = self.instance_masks[tokens_ids_to_use_cvt]
            example["recon_masks"] = self.instance_masks[tokens_ids_to_use_cvt]
        else:
            tokens_ids_to_use = random.sample(range(1, len(self.placeholder_tokens)), k=1) * 2  
            # tokens_to_use = [self.placeholder_tokens[tokens_ids_to_use[0]], self.placeholder_relation_tokens[self.relation_dict.get(frozenset([tokens_ids_to_use[0], tokens_ids_to_use[0]]),None)], self.placeholder_tokens[tokens_ids_to_use[0]]]
            tokens_to_use = [self.placeholder_tokens[tokens_ids_to_use[0]], self.placeholder_tokens[tokens_ids_to_use[0]]]
            tokens_ids_to_use_cvt = [token_id-1 for token_id in tokens_ids_to_use]
            if self.num_of_fragments_list[tokens_ids_to_use[0]] == 0:
                example["prompt_masks"] = self.instance_masks[tokens_ids_to_use_cvt]
                example["recon_masks"] = self.instance_masks[tokens_ids_to_use_cvt]
            else:
                fragment_id_to_use = random.randrange(0,self.num_of_fragments_list[tokens_ids_to_use[0]])
                example["prompt_masks"] = torch.stack([self.instance_masks[tokens_ids_to_use_cvt][0], self.instance_fragment_masks[tokens_ids_to_use_cvt[0]][fragment_id_to_use].squeeze(0)])
                example["recon_masks"] = torch.stack([self.instance_masks[tokens_ids_to_use_cvt][0], self.instance_fragment_masks[tokens_ids_to_use_cvt[0]][fragment_id_to_use].squeeze(0)])

        example["use_generated_image"] = True
        if example["use_generated_image"] and not os.path.exists(os.path.join(self.instance_data_root, 'gen_img')):
            raise FileNotFoundError('Please generate images before training.')  
        example["use_generated_image"] = (random.random() < self.gen_p) 
        #if not example["use_generated_image"]:
        example["instance_images"] = self.instance_image
        '''
        else:
            sample_idx = random.randrange(0,5)
            binary_list = ['0'] * (self.num_of_assets-1)
            for k in range(len(tokens_ids_to_use)):
                if tokens_ids_to_use[k] == 0:
                    continue
                binary_list[tokens_ids_to_use[k]-1] = '1'
            binary_string = ''.join(binary_list)
            generated_img_path = os.path.join(self.instance_data_root, 'gen_img', binary_string, f"{sample_idx}.jpg")
            example["instance_images"] = self.image_transforms(Image.open(generated_img_path))
        '''
        ...
        if random.random() > self.flip_p:
            ...
            example["prompt_masks"] = TF.hflip(example["prompt_masks"])
            example["recon_masks"] = TF.hflip(example["recon_masks"])
...
def collate_fn(examples, with_prior_preservation=False):
    ...
    recon_masks = [example["recon_masks"] for example in examples]
    prompt_masks = [example["prompt_masks"] for example in examples]
    ...
    recon_masks = torch.stack(recon_masks)
...
class SpatialDreambooth:
    ...
    def main(self):
        ...
        # Auto-detect num_of_assets if not specified or if it's the default value
        if self.args.num_of_assets == 1:  # Default value, likely not manually set
            detected_num_of_assets = auto_detect_num_of_assets(self.args.instance_data_dir)
            logger.info(f"Auto-detected num_of_assets: {detected_num_of_assets}")
            self.num_of_assets = detected_num_of_assets
            # Update args for consistency
            self.args.num_of_assets = detected_num_of_assets
        else:
            self.num_of_assets = self.args.num_of_assets
            logger.info(f"Using provided num_of_assets: {self.num_of_assets}")
        ...
        # Add relations tokens to tokenizer
        # Auto-detect fragments from mask files
        self.num_of_fragments = auto_detect_fragments(self.args.instance_data_dir, self.args.num_of_assets)
        logger.info(f"Auto-detected fragments: {self.num_of_fragments}")
        assert (len(self.num_of_fragments) == self.args.num_of_assets)
        self.relation_dict = {}
        self.placeholder_relation_tokens = []
        self.edge_cnt = 0
        # for i in range(self.args.num_of_assets):
        #     for j in range(i, self.args.num_of_assets):
        #         self.placeholder_relation_tokens.append(self.args.placeholder_relation_token.replace(">", f"{self.edge_cnt}>"))
        #         self.relation_dict[frozenset([i,j])] = self.edge_cnt
        #         self.edge_cnt = self.edge_cnt+1
                
        # num_added_relations = self.tokenizer.add_tokens(self.placeholder_relation_tokens)  
        # self.placeholder_relation_token_ids = self.tokenizer.convert_tokens_to_ids(   
        #     self.placeholder_relation_tokens
        # )
        # self.text_encoder.resize_token_embeddings(len(self.tokenizer))
        ...
        # # Initialize new tokens randomly
        # token_embeds = self.text_encoder.get_input_embeddings().weight.data
        # token_embeds[-self.edge_cnt :] = token_embeds[
        #     -3 * self.edge_cnt : -2 * self.edge_cnt
        # ]
        ...
        train_dataset = DreamBoothDataset(
            ...
            placeholder_relation_tokens=self.placeholder_relation_tokens,
            class_data_root=None,
            ...
            num_of_fragments =self.num_of_fragments,
        )
        ...
        for epoch in range(first_epoch, self.args.num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                with self.accelerator.accumulate(self.unet):
                    ...
                    # No chunk No prior loss
                    if self.args.apply_masked_loss:
                        max_masks = torch.max(
                            batch["recon_masks"], axis=1
                        ).values
                        ...
                        model_pred_partial = model_pred * downsampled_mask
                        target_partial = target * downsampled_mask
                    loss = F.mse_loss(
                        model_pred.float(), target.float(), reduction="mean"
                    )
                    partial_loss = F.mse_loss(
                        model_pred_partial.float(), target_partial.float(), reduction="mean"
                    )
                    # Add the prior loss to the instance loss.
                    loss = self.args.whole_loss_weight * loss + self.args.masked_loss_weight * partial_loss
                    ...
                    if self.args.lambda_attention != 0:
                        ...
                        for batch_idx in range(self.args.train_batch_size):
                            GT_masks = F.interpolate(
                                input=batch["prompt_masks"][batch_idx], size=(16, 16)
                            )
                            ...
                            for mask_id in range(len(GT_masks)):
                                ...
                                try:
                                    asset_idx = ( (batch["input_ids"][curr_cond_batch_idx] == curr_placeholder_token_id).nonzero().item())
                                except:
                                    # for v1 v1 situation
                                    asset_idx = ( (batch["input_ids"][curr_cond_batch_idx] == curr_placeholder_token_id).nonzero()[mask_id].item())
                            ...
                            # GT_masks_merge = GT_masks.sum(dim=0).unsqueeze(0)
                            # GT_masks_merge[GT_masks_merge>=1] = 1
                            # curr_placeholder_relation_token_id = self.placeholder_relation_token_ids[
                            #     self.relation_dict.get(frozenset([int(batch["token_ids"][batch_idx][0]), int(batch["token_ids"][batch_idx][1])]),None)
                            #     ]
                            # asset_idx = (
                            #     (
                            #         batch["input_ids"][curr_cond_batch_idx]
                            #         == curr_placeholder_relation_token_id
                            #     )
                            #     .nonzero()
                            #     .item()
                            # )
                ...
                if self.accelerator.sync_gradients:
                    ...
                    if (
                        ...
                        if self.args.lambda_attention != 0:
                            ...
                            self.tokenizer.decode(batch["input_ids"][curr_cond_batch_idx])
...
class P2PCrossAttnProcessor:
    ...
    def __call__(
        ...
        attention_mask = attn.prepare_attention_mask(attention_mask, batch_size, sequence_length)
