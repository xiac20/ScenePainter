...
from models.BLD import BlendedLatnetDiffusion
from util.finetune_utils import finetune_depth_model, finetune_decoder, finetune_customization
...
def evaluate_epoch(model, epoch, vmax=None):
    ...
    (save_root / "regen").mkdir(exist_ok=True, parents=True)
    (save_root / "regen_inpaint").mkdir(exist_ok=True, parents=True)
...
def run(config):
    ...
    if config['skip_gen']:
        kfgen_save_folder = Path(config['runs_dir']) / f"{config['kfgen_load_dt_string']}/kfgen"
    else:
        dt_string = datetime.now().strftime("%m-%d_%H-%M-%S")
        base_dir = Path(config['runs_dir']) / f"{dt_string}"
        kfgen_save_folder = Path(config['runs_dir']) / f"{dt_string}" / "A_Kfgen"
    ...
    all_rundir = []
    start_keyframe = Image.open(config['image_filepath']).convert('RGB').resize((512, 512))
    content_prompt, style_prompt, adaptive_negative_prompt, background_prompt, control_text = config['content_prompt'], config['style_prompt'], config['negative_prompt'], config.get('background', None), config.get('control_text', None)
    ...
    inpainter_pipeline = StableDiffusionInpaintPipeline.from_pretrained(
            config["stable_diffusion_inpainting_checkpoint"],
            safety_checker=None,
    ...
    vae = AutoencoderKL.from_pretrained(config["stable_diffusion_inpainting_checkpoint"], subfolder="vae").to(config["device"])
    bld = BlendedLatnetDiffusion(config["device"], config["stable_diffusion_customization_checkpoint"])
    ...
    for i in range(config['num_scenes']):
        ...
        for j in range(config['num_keyframes']):
            ...
            if config['skip_gen']:
                ...
                for regen_id in range(3): 
                    ...
                    kf_gen = KeyframeGen(config, inpainter_pipeline, mask_generator, depth_model, vae, rotation, 
                                        start_keyframe, base_dir, i, j, regen_negative_prompt + adaptive_negative_prompt,
                                        segment_model=segment_model, segment_processor=segment_processor, BLD = bld).to(config["device"])
                    ...
                    ToPILImage()(render_output['rendered_image'][0]).save(save_root / "regen" / f"{regen_id}.png")
                    inpaint_output = kf_gen.inpaint(render_output["rendered_image"], render_output["inpaint_mask"], num_of_assets=config['num_of_assets'], style_prompt=style_prompt, negative_prompt=config['negative_inpainting_prompt'])
                    ToPILImage()(inpaint_output['inpainted_image'][0]).save(save_root / "regen_inpaint" / f"{regen_id}.png")
                ...
                if config["finetune_decoder_customization"]:
                    ToPILImage()(inpaint_output["inpainted_image"].detach()[0]).save(save_root / 'kf2_before_ft.png')
                    finetune_customization(config, kf_gen, render_output, inpaint_output, config['num_finetune_customization_steps'])
            ...
            kf_interp = KeyframeInterp(config, inpainter_pipeline, None, vae, rotation, 
                                   ToPILImage()(kf1_image[0]),  base_dir, i, j,adaptive_negative_prompt,
